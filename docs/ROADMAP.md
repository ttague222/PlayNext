...
- **1.4.0 feature set implemented 2026-09-22 on branch `worktree-1-4-0-engagement`** (subagent-driven, two-stage reviewed) — NOT merged or released yet: post-update changelog modal (announces 1.3.0+1.4.0 features to upgraders; version-stamp logic distinguishes pre-tracking upgraders from fresh installs), Coming Soon section on What's New backed by a `release_date` field + `/games/upcoming` endpoint, engine excludes unreleased games, thumbs ratings (`rated_up`/`rated_down` signals feed engine exclusion + taste nudges; % liked shown at ≥10 ratings via GET/PUT `/signals/game/{id}/rating`), weekly digest now mentions coming-soon titles (announced-only weeks send too — note: once upcoming titles exist, effectively all enabled devices get the weekly digest; re-engagement branch is dormant). Monthly new-release candidate pipeline: `scripts/new_release_candidates.py` + `new-release-candidates.yml` opens a curation PR on the 5th monthly (needs `RAWG_API_KEY` repo secret + "Allow GitHub Actions to create and approve PRs" setting); `seed_refresh.py` now refuses uncurated entries and passes `release_date`/`fun_fact` through. Fall 2026 batch drafted (15 titles, 10 future-dated) in `games_data/refresh_2026_fall.json` — NOT seeded; Tom must review curation + verify titles/dates, then seed close to the 1.4.0 release (old binaries deep-link announced-only pushes to a What's New with no Coming Soon section), then run `migrate_field_names.py` (the transform writes `year`, the Game model needs `release_year` — detail screens 404 for new titles until the migration runs). Ship gates for 1.4.0: version bump (`app.config.js` still reads 1.3.0 — the changelog modal is inert until bumped), store What's New copy, API deploy BEFORE the binary reaches users, device pass via `/mobile-release`
