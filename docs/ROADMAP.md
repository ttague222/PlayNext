...
- **1.4.0 merged to main, API LIVE, catalog seeded, emulator-validated — build/submit pending** (2026-09-23, merge 5af603c): post-update changelog modal (announces 1.3.0+1.4.0 to upgraders; version-stamp logic keeps fresh installs quiet), Coming Soon on What's New backed by `release_date` + `/games/upcoming`, engine excludes unreleased games, thumbs ratings (`rated_up`/`rated_down` feed exclusion + taste nudges; % liked at >=10 ratings; no rating row on unreleased games), weekly digest mentions coming-soon titles (announced-only pushes capped to days 1-7 of the month; with upcoming titles seeded, all enabled devices get the weekly digest). Fall batch SEEDED (15 titles incl. GTA VI 2026-11-19; `migrate_field_names.py` run, `created_at` stamped — `seed_refresh.py` now writes timestamps + refuses uncurated entries), so this Saturday's digest announces 5 new games. Monthly candidate pipeline LIVE: first manual run opened PR #10 (3 candidates awaiting curation; dedupe verified against the fresh seed); runs the 5th monthly; `RAWG_API_KEY` secret + Actions PR setting configured. Versions bumped (1.4.0 / iOS build 16 / versionCode 23), store What's New copy in `store/` (Play copy carries the 1.3.0 Steam-sync news Play users never saw), settings footer version fixed. Emulator-validated end to end (fresh-install guard, pick flow + signal loop, rating toggle + server persistence). **Remaining**: `eas build --platform all --profile production`, device pass (changelog modal + Coming Soon happen naturally on a 1.3.0->1.4.0 phone), submit; checklist in `docs/runbooks/ship-1-4-0.md`. On ASC, label the new version **1.4.0** — the live listing shows 1.0.7, so the label track never converged
